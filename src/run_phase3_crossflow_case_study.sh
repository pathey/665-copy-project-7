...
TEMPLATE="./config/cfg/phase3_case_study_crossflow_template.yaml"
...
for mcs in "${mcs_modes[@]}"; do
  for rate in "${rates[@]}"; do
	run_name="phase3_crossflow_${distance}m_mcs${mcs}_${rate}Mbps"
...
for dec_threshold in "${decrement_thresholds[@]}"; do
  for inc_threshold in "${increment_thresholds[@]}"; do
    for rate in "${rates[@]}"; do
      ...
      run_name="phase3_crossflow_${distance}m_mcs_adaptive_${rate}Mbps_${inc_threshold}inc_${dec_threshold}dec"
      out_cfg="${TMP_DIR}/${run_name}.yaml"
