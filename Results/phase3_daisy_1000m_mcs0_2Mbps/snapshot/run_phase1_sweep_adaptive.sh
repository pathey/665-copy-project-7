...
TEMPLATE="./config/cfg/phase1_daisy_template.yaml"
...
for distance in "${distances[@]}"; do
  for threshold in "${increment_thresholds[@]}"; do
    for rate in "${rates[@]}"; do
      ...
      run_name="phase1_daisy_${distance}m_mcs_adaptive_${rate}Mbps_${threshold}increment_threshold"
...
for distance in "${distances[@]}"; do
  for threshold in "${decrement_thresholds[@]}"; do
    for rate in "${rates[@]}"; do
      ...
      run_name="phase1_daisy_${distance}m_mcs_adaptive_${rate}Mbps_${threshold}decrement_threshold"
      out_cfg="${TMP_DIR}/${run_name}.yaml"
